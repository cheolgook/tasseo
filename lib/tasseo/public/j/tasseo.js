...
TasseoMetric.prototype = {
  clear: function() {
    ...
    this.graph = new Rickshaw.Graph({
      ...
      min: this.ymin || this.dashboardOptions.ymin
    });
...
function getRegexForWildcard(pattern) {
  return new RegExp(pattern.replace(/\(/g,'\\(').replace(/\)/g,'\\)').replace(/\-/g,'\\-').replace(/\./g,'\\.').replace(/\*/g,'.*').replace(/\?/g,'.'));
}
...
TasseoGraphiteDatasource.prototype = {
    ...
    $.ajax({
      ...
      _.each(metrics, function(metric, idx) {
        var metricResult = _.find(metricResults, function(metricResult) {
          if(isWildcard(metric.target)) {
            return getRegexForWildcard(metric.target).test(metricResult.target);
          }
          return self.padString(metric.target) == metricResult.target;
        });
  ...
  urlForMetrics: function(metrics, period) {
    var targets = _.map(metrics, function(metric) {
      return 'target=' + this.padString(encodeURI(metric.target));
    }, this).join("&");
